...
ngx_stream_request_t* parse_request_handler (ngx_stream_session_t* s);
void build_response_handler(ngx_stream_request_t* r);
...
static ngx_stream_request_t* parse_handshake(ngx_stream_session_t* s) {
  ...
  ngx_log_error(NGX_LOG_ERR, log, 0, "%*s", re, ctx->recv_buffer->last);
...
static ngx_stream_request_t* parse_request(ngx_stream_session_t* s) {
  ...
  request_ctx* extra = ngx_stream_request_get_module_ctx(r, this_module);
  extra->reqid = ntohl(*(uint32_t*)reqid);
  ngx_log_error(NGX_LOG_INFO, s->connection->log, 0, "websocket reqid = %ud", extra->reqid);
...
#define PUSH_REQ_ID 1 // need equal client

static void build_websocket_content_protocol(ngx_stream_request_t* r) {
  ...
  const uint32_t pushID = PUSH_REQ_ID;
...
static void build_response(ngx_stream_request_t* r) {
  ...
  switch (r_ctx->type) {
    ...
    default: // BINARY
      enc_data(r);
      build_websocket_content_protocol(r);
