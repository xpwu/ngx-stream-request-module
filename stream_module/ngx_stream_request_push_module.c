...
static void push_init_parser(ngx_stream_session_t*);
static ngx_stream_request_t* push_parse_request(ngx_stream_session_t*);
static void push_build_response(ngx_stream_request_t*);
static void push_init_proxy_handler(ngx_stream_request_t*);
static void push_proxy_handle_request(ngx_stream_request_t*);
...
static char *ngx_stream_request_push_merge_srv_conf(ngx_conf_t *cf
  ...
  if (conf->is_push) {
    ...
    cmcf = ngx_stream_conf_get_module_main_conf(cf, core_module);
    void (** p)(ngx_stream_session_t*) = ngx_array_push(&cmcf->session_initializers);
    *p = push_initializer;
  }
...
static char *push_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
  ...
  cscf->init_parser = push_init_parser;
  cscf->build_response = push_build_response;
  cscf->parse_request = push_parse_request;
  cscf->proxy_handle_request = push_proxy_handle_request;
  cscf->init_proxy_handler = push_init_proxy_handler;
  
  ngx_stream_request_push_svr_conf_t* pscf = conf;
  pscf->is_push = 1;
...
static void push_initializer(ngx_stream_session_t* s) {
  ngx_stream_request_push_svr_conf_t* pscf;
  ...
  ngx_stream_request_push_token_t token = {
    pmcf->hostname, ngx_process_slot, ngx_pid, r_ctx->time_stamp, r_ctx->session_seq
  };
  ngx_str_t token_str;
  u_char str[2*sizeof(ngx_stream_request_push_token_t)];
  token_str.data = str;
  token_str.len = 0;
...
static void push_init_parser(ngx_stream_session_t* s){
...
static ngx_stream_request_t* push_parse_request(ngx_stream_session_t* s) {
...
static void push_build_response(ngx_stream_request_t* r) {
...
static void push_init_proxy_handler(ngx_stream_request_t* r){}
...
static void push_proxy_handle_request(ngx_stream_request_t* r) {
  ...
  if (r_ctx->token.hostname != pmcf->hostname) {
    ngx_log_error(NGX_LOG_NOTICE, log, 0, "r(%p) hostname(%uD) != local hostname(%uD)"
                  , r, r_ctx->token.hostname, pmcf->hostname);
    create_failed_request(r, 1);
  ...
  if (r_ctx->token.pid != (ngx_processes[r_ctx->token.slot].pid&0xffff)) {
    ...
    ngx_log_error(NGX_LOG_NOTICE, log, 0, "free share memory");
  ...
  if (r_ctx->token.slot == ngx_process_slot) {
    if (find_session(pmcf->sessions, r_ctx->token.time_stamp
      ...
      ngx_log_error(NGX_LOG_ERR, log, 0, "r(%p) not find session", r);
      create_failed_request(r, 2);
    }
    handle_request_done(r);
    // call function whether or not find_session==null
    push_msg_to_client();
    return;
  }
...
static ngx_stream_request_t* request_parse_header(ngx_stream_session_t* s) {
  ...
  ngx_core_conf_t *ccf = (ngx_core_conf_t *) ngx_get_conf(ngx_cycle->conf_ctx
                                                          , ngx_core_module);
  ngx_int_t workers = ccf->worker_processes;
  if (r_ctx->token.slot >= workers) { // 数据丢弃
      ...
      case NGX_STREAM_PUSH_CHANNEL_CMD_RESPONSE:
        {
          ...
          handle_request_done((ngx_stream_request_t*)r);
        }
        break;
    }
  }
...
static ngx_int_t ngx_stream_push_read_channel(ngx_socket_t s
  ...
  if ((size_t) n < sizeof(ngx_stream_push_channel_t)) {
