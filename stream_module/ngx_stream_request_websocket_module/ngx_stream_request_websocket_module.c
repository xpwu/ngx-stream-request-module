...
static ngx_int_t parse_handshake(ngx_stream_request_t*);
...
static ngx_int_t parse_request(ngx_stream_request_t* r);
...
//static ngx_int_t* parse_request_handler (ngx_stream_session_t* s);
//static ngx_int_t build_response_handler(ngx_stream_request_t* r);
...
char *websocket_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
  ...
  cscf->protocol.handler.name = "websocket";
...
static void init_parse(ngx_stream_session_t* s) {
  ...
  ctx->handle_request = parse_handshake;
...
static ngx_int_t parse_handshake(ngx_stream_request_t* r) {
...
static void init_parse_request(ngx_stream_session_t* s) {
  ...
  ctx->handle_request = parse_request;
...
static void build_websocket_v13_close(ngx_stream_request_t* r, ngx_uint_t close_code) {
  r->data->buf = ngx_create_temp_buf(r->pool, 4);
  r->data->next = NULL;
  u_char *buf = r->data->buf->last;
  buf[0] = 0x88;
  buf[1] = 0x02;
...
static ngx_int_t parse_request(ngx_stream_request_t* r) {
  request_ctx* ctx = ngx_stream_request_get_module_ctx(r, this_module);
