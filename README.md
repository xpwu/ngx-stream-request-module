...
##简介
实现客户端到服务器的长连接通信, 客户端与服务器之间仅需一条长连接就可实现客户端到服务器get数据,以及服务器向客户端push数据. 无需更改服务器逻辑即可实现现有的http短连接方式升级到长连接. 支持 ios android web 微信小程序，也支持ssl连接
...
##客户端
客户端主要有4个接口, 调用其中两个即可实现和服务器的通信, 调用简单.
